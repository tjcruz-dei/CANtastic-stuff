 ...
 * 
 *  Original version written By: Cory J. Fowler  December 20th, 2016 - OBD PID support
...
// Shall we pollute the terminal with debugging crap?
// comment if you want to disable the debug feed
#define DEBUG

// super-lazy approach to debugging - borderline clever
#ifdef DEBUG
#define DBG(complain) \
    Serial.println(complain)
#else
#define DBG(complain) ((void)0)
#endif

//mapped register bank refresh
#define REFRESHTIME 5000

// buffer for translated OBD parameters
// can be imploded easily after a timeout
// isolated variables are in fact pointers to the buffer
uint8_t obdParams[10];
uint8_t *const speed=obdParams;
uint16_t *const rpm=(uint16_t*)(obdParams+1);
...
//Internal CAN bus side
// FIRST MCP
// A0->CS
// A1->INT
#define CAN0_INT A1                              // Set CAN0 INT to pin 2
MCP_CAN CAN0(A0);                               // Set CAN0 CS to pin 10

// OBD CAN side
// SECOND MCP
// 2 -> INT
// 7 -> CS
#define CAN1_INT 2                              // Set CAN0 INT to pin 2
MCP_CAN CAN1(7);                               // Set CAN0 CS to pin 10
...
void setup()
{
  ...
  if(CAN0.begin(MCP_STDEXT, CAN_125KBPS, MCP_8MHZ) == CAN_OK)
    DBG("MCP2515 CAN0 Initialized Successfully!");
  else
    DBG("Error Initializing MCP2515...");

    // Initialize MCP2515 running at 16MHz with a baudrate of 500kb/s and the masks and filters disabled.
  if(CAN1.begin(MCP_STDEXT, CAN_500KBPS, MCP_8MHZ) == CAN_OK)
    DBG("MCP2515 CAN1-OBD Initialized Successfully!");
  else
    DBG("Error Initializing MCP2515...");

#if standard == 1
  // Standard ID Filters
  CAN1.init_Mask(0,0x7F00000);                // Init first mask...
  CAN1.init_Filt(0,0x7DF0000);                // Init first filter...
  CAN1.init_Filt(1,0x7E10000);                // Init second filter...
  
  CAN1.init_Mask(1,0x7F00000);                // Init second mask... 
  CAN1.init_Filt(2,0x7DF0000);                // Init third filter...
  CAN1.init_Filt(3,0x7E10000);                // Init fourth filter...
  CAN1.init_Filt(4,0x7DF0000);                // Init fifth filter...
  CAN1.init_Filt(5,0x7E10000);                // Init sixth filter...

  //OBD-CAR Filters
  CAN0.init_Filt(1,0xB600000); // Engine speed and rpm
  CAN0.init_Filt(2,0x1280000); // Light cluster             


#else
  // Extended ID Filters
  CAN1.init_Mask(0,0x90FFFF00);                // Init first mask...
  CAN1.init_Filt(0,0x90DB3300);                // Init first filter...
  CAN1.init_Filt(1,0x90DA0100);                // Init second filter...
  
  CAN1.init_Mask(1,0x90FFFF00);                // Init second mask... 
  CAN1.init_Filt(2,0x90DB3300);                // Init third filter...
  CAN1.init_Filt(3,0x90DA0100);                // Init fourth filter...
  CAN1.init_Filt(4,0x90DB3300);                // Init fifth filter...
  CAN1.init_Filt(5,0x90DA0100);                // Init sixth filter...
#endif
...
void loop()
{
  if(!digitalRead(CAN0_INT)){                  // If interrupt pin is low, read CAN0 receive buffer
    updateInt=millis();
    //DBG("CAN0 receive");
    CAN0.readMsgBuf(&rxId, &dlc, rxBuf);       // Read data: dlc = data length, buf = data byte(s)
    ...
    if (rxId==0xB6) {
        ...
        *rpm=*rpm|(rxBuf[1] >> 3);
        DBG("RPM:"+String(*rpm));
        uint16_t CANspeed= ((rxBuf[2]<<8));
        CANspeed= ((CANspeed | rxBuf[3]) & 0xFFF0);
        *speed=CANspeed/100;
        DBG("True CAN Speed:"+String(*speed));
        DBG("Biased Quadrant Speed:"+String(*speed*1.02));
    }
  }
  ...
  if ((millis()-updateInt)>REFRESHTIME){
    updateInt=millis();
    memset(obdParams, 0, sizeof(obdParams));
  }
}
...
void obdReq(byte *data){
  ...
  if(mode == 0x01){
    if(pid == 0x00){        // Supported PIDs 01-20
      ...
      txData[3] = 0x88; //adicionado 0x08 para teste
      txData[4] = 0x38;
      txData[5] = 0x00;
      txData[6] = 0x01;
      tx = true;
...
//    }
    else if(pid == 0x67){    // Engine coolant temperature TJC
       txData[0] = 0x03;
      
      txData[3] = 0xFA;
      tx = true;
    }
    else if(pid == 0x68){    // Intake air temperature sensor TJC
       txData[0] = 0x03;
...
//    packetcnt++;
  ...
  while(not_done){
    ...
    if(!digitalRead(CAN1_INT)){
      ...
      //Flow control frame detection, per ISO-TP CAN (FC)
